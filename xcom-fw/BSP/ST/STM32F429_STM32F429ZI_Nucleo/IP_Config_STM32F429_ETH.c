...
**********************************************************************
----------------------------------------------------------------------
File        : IP_Config_STM32F429_ETH.c
Purpose     : Configuration file for TCP/IP with STM32F429
---------------------------END-OF-HEADER------------------------------
*/
...
#define TARGET_NAME  "STM32F429"                 // Target name used for DHCP client.
#define HW_ADDR      "\x00\x22\xC7\xFF\xFF\xFE"  // MAC addr. used for target.
...
//
#define IP_ADDR      IP_BYTES2ADDR(192, 168,   2, 22)
...
//
#ifdef __ICCARM__
  #pragma location="IP_RAM"
  static __no_init U32 _aPool[ALLOC_SIZE / 4];
#elif __GNUC__
  static U32 _aPool[ALLOC_SIZE / 4] __attribute__ (( section (".RAM1") ));
#else
...
void IP_X_Config(void) {
  ...
  //
  // Configure MAC address using jumpers
  //
  
  // Pins to read
  int ReadPD6 = GPIO_ReadInputDataBit(GPIOD,GPIO_Pin_6);
  int ReadPD7 = GPIO_ReadInputDataBit(GPIOD,GPIO_Pin_7);

  // Make and assign the MAC address
  U8 hexValue = (ReadPD7 << 1) | ReadPD6;
  const U8 macaddr[] = {0x00, 0x22, 0x00, 0xFF, 0xFF, hexValue};
  IP_SetHWAddrEx(IFaceId, macaddr, 6);
