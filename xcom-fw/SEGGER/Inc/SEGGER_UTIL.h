...
// You'd think the intrinsic functions would do a better job
// of byte swapping, maybe using native x86 instructions but...
// you would be mistaken.
//
// #if defined(_MSC_VER) && (_MSC_VER >= 1900)  // MS Visual Studio 2015 on
//   #include <stdlib.h>
//   #define SEGGER_BSWAP_U16             _byteswap_ushort
//   #define SEGGER_BSWAP_U32             _byteswap_ulong
//   #define SEGGER_BSWAP_U64             _byteswap_uint64
// #endif
...
static INLINE U32 SEGGER_RdU24LE(const U8 aData[3]) {
  return (U32)SEGGER_RdU16LE(aData) + ((U32)aData[2] << 16);
}
...
static INLINE U32 SEGGER_RdU24BE(const U8 aData[3]) {
  return SEGGER_RdU16BE(&aData[1]) + (U32)aData[0]*0x10000u;
}
...
static INLINE void SEGGER_WrU24BE(U8 aData[], U32 Value) {
  aData[0] = (U8)(Value >> 16);
  SEGGER_WrU16BE(&aData[1], (U16)Value);
}
