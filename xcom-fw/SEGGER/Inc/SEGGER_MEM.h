...
typedef void SEGGER_MEM_LOCK_UNLOCK_FUNC(void *pContext);

typedef struct {
  SEGGER_MEM_LOCK_UNLOCK_FUNC *pfLock;
  SEGGER_MEM_LOCK_UNLOCK_FUNC *pfUnlock;
} SEGGER_MEM_LOCK_API;


typedef struct {
  const SEGGER_MEM_API      * pAPI;
  const SEGGER_MEM_LOCK_API * pLockAPI;
        void                * pContext;
        void                * pLockContext;
} SEGGER_MEM_CONTEXT;
...
typedef int SEGGER_MEM_PRINTF_FUNC(const char *sFormat, ...);
