...
TCPCB *  IP_TCP_FindExactMatch (U32 FAddr, U16 FPort, U32 LAddr, U16 LPort);

//
// TCP timer
//
void     IP_TCP_Timer(void);
...
int  IP_TCP_SOCK_Recv        (SOCKET* pSock, char* pBuffer, int NumBytes, int Flags, int (*pfOutput)(TCPCB* pTCPCB));
int  IP_TCP_SOCK_Send        (SOCKET* pSock, const U8* pData, int NumBytes, int (*pfOutput)(TCPCB* pTCPCB));
